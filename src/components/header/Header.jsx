...
const Header = () => {
    return (
        <header className="relative bg-black bg-opacity-50 w-screen h-screen flex flex-col justify-between items-center" id='home'>
            <Suspense fallback={<div className='absolute inset-0 -z-10 w-full h-full text-6xl text-red-600'>Loading</div>}>
                <HeaderVideo />
            </Suspense>
            <div className='flex flex-col flex-4 justify-center items-center gap-4 pt-8 w-full text-white'>
                ...
                <HashLink to='/#contact-us' className='border-2 border-white px-8 py-4 mt-8 text-lg uppercase  hover:bg-sky-600 hover:border-sky-600 rounded-lg  hover:rounded-3xl transition-all duration-500'>Contact Us</HashLink>
            </div>
